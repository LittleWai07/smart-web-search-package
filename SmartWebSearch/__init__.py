...
class SmartWebSearch:
    ...
    def __init__(self, ts_api_key: str, openai_comp_api_key: str, model: str = "deepseek-chat", openai_comp_api_base_url: str = "https://api.deepseek.com/chat/completions") -> None:
        ...
        def rag_progress_listener(progress_data: _ProgressData) -> None:
            ...
            match progress_data.status:
                case 'KL_BASE_CREATING':
                    self.progress._update_progress('KL_BASE_CREATING', progress_data.message, progress_data.data, progress_data.progress)
                case 'KL_BASE_CREATED':
                    self.progress._update_progress('KL_BASE_CREATED', progress_data.message, progress_data.data, progress_data.progress)
                case 'KL_BASE_MATCHING':
                    self.progress._update_progress('KL_BASE_MATCHING', progress_data.message, progress_data.data, progress_data.progress)
                case 'KL_BASE_MATCHED':
                    self.progress._update_progress('KL_BASE_MATCHED', progress_data.message, progress_data.data, progress_data.progress)
                case 'COMPLETED':
                    self.progress._update_progress('PART_COMPLETED', progress_data.message, progress_data.data, progress_data.progress)
    ...
    def search(self, prompt: str, stream_cb: Callable[[str], None] = None) -> str:
        ...
        def ts_progress_listener(progress_data: _ProgressData) -> None:
            ...
            match progress_data.status:
                case 'SEARCHING':
                    self.progress._update_progress('SEARCHING', progress_data.message, progress_data.data, progress_data.progress)
                case 'SEARCHED':
                    self.progress._update_progress('SEARCHED', progress_data.message, progress_data.data, progress_data.progress)
                case 'PARSING':
                    self.progress._update_progress('PARSING', progress_data.message, progress_data.data, progress_data.progress)
                case 'PARSED':
                    self.progress._update_progress('PARSED', progress_data.message, progress_data.data, progress_data.progress)
                case 'PART_COMPLETED':
                    self.progress._update_progress('PART_COMPLETED', progress_data.message, progress_data.data, progress_data.progress)
                case 'COMPLETED':
                    self.progress._update_progress('PART_COMPLETED', progress_data.message, progress_data.data, progress_data.progress)
        ...
        # Update progress
        self.progress._update_progress('STORMING', f"Storming the main queries and auxiliary queries for the prompt '{prompt}'")

        # Generate some search queries
        m_query, *a_queries = self.qs.storm_with_prompt(prompt)

        # Update progress
        self.progress._update_progress('STORMED', f"Stormed the main queries and auxiliary queries for the prompt '{prompt}'", {
            'main_query': m_query,
        ...
        # Update progress
        self.progress._update_progress('CONCLUDING', f"Concluding the content for the prompt '{prompt}'")

        # Summarize the content
        conclusion = self.smr.summarize(prompt, content, stream_cb)

        # Update progress
        self.progress._update_progress('CONCLUDED', f"Concluded the content for the prompt '{prompt}'", {
            'prompt': prompt,
            'summaries': [ result.summary for result in results ],
            'conclusion': conclusion
        })

        self.progress._update_progress('COMPLETED', f"Search completed for the prompt '{prompt}'", {
            'prompt': prompt,
            'summaries': [ result.summary for result in results ],
            'conclusion': conclusion
        })
    ...
    def deepsearch(self, prompt: str, stream_cb: Callable[[str], None] = None) -> str:
        ...
        # Update progress
        self.progress._update_progress('STORMING', f"Decomposing the prompt '{prompt}' into tasks")

        # Decompose the prompt into tasks
        tasks: list[str] = self.qs.decompose_tasks_with_prompt(prompt)

        # Update progress
        self.progress._update_progress('STORMED', f"Decomposed the prompt '{prompt}' into tasks", {
            'tasks': tasks
        })
        ...
        for task in tasks:
            # Update progress
            self.progress._update_progress('STORMING', f"Storming the main queries and auxiliary queries for the task '{task}'")
            ...
            # Update progress
            self.progress._update_progress('STORMED', f"Stormed the main queries and auxiliary queries for the task '{task}'", {
                'main_query': m_query,
                'auxiliary_queries': a_queries
            })
        ...
        # Update progress
        self.progress._update_progress('CONCLUDED', f"Concluded the summaries and matches for the prompt '{prompt}'", {
            'prompt': prompt,
            'summaries': src.get_summaries(),
            'matches': matches,
            'conclusion': conclusion
